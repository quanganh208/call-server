...
io.on("connection", (socket) => {
        ...
        if (targetAdmin) {
            ...
            if (targetAdminSocket) {
                ...
                const adminCallTimeout = setTimeout(() => {
                    console.log(`Admin call from ${socket.id} to ${targetAdminId} timed out`);

                    // Notify calling admin about timeout
                    socket.emit("admin-call-timeout", {
                        targetAdminId: targetAdminId,
                        adminName: adminSockets.get(targetAdminId)?.name || "Admin",
                        message: "Call request timed out. The admin didn't answer your call."
                    });
    ...
    socket.on("end-call", (data) => {
        ...
        // Send call end notification to partner
        if (data.targetId) {
            const targetSocket = io.sockets.sockets.get(data.targetId);
            if (targetSocket) {
    ...
    socket.on("cancel-admin-call", (data) => {
        ...
        console.log(`Admin ${socket.id} cancelled call to admin ${data.targetAdminId}`);
    ...
    socket.on("disconnect", () => {
        ...
        if (socket.role === "client") {
            ...
            for (const [callId, callData] of activeCallRequests.entries()) {
                if (callData.type === "admin-admin" &&
                    (callData.callerAdminId === socket.id || callData.targetAdminId === socket.id)) {
                    clearTimeout(callData.timeout);
                    activeCallRequests.delete(callId);

                    // Notify the other admin if needed
                    const otherAdminId = callData.callerAdminId === socket.id ?
                        callData.targetAdminId : callData.callerAdminId;
